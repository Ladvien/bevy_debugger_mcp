/*
 * Bevy Debugger MCP Server - Bevy Reflection Integration
...
use crate::brp_messages::{ComponentValue, EntityData};
...
impl BevyReflectionInspector {
    ...
    async fn inspect_value_generic(&self, value: &ComponentValue, type_name: &str) -> Result<InspectedValue> {
        let inspected = match value {
            Value::Object(obj) => {
                let mut children = Vec::new();
                for (key, val) in obj {
                    let child = self.inspect_value_generic(val, "unknown").await?;
    ...
    fn assess_change_severity(&self, old_value: &Value, new_value: &Value) -> ChangeSeverity {
...
// Custom inspector implementations for common Bevy types
